...
-(void)copyDatabaseIntoDocumentsDir;
...
-(instancetype)initWithDatabaseFilename:(NSString *)dbFilename
{
    self = [super init];
    if(self){
        // Set the documents directory path to the documents directory property
        ...
        [self copyDatabaseIntoDocumentsDir];
    ...
    if(openDatabaseResult == SQLITE_OK) {
        ...
        if(prepareStatementResult == SQLITE_OK) {
            ...
            if (!queryExecutable){
                ...
                // Loop through the results and add them to the array row by row.
...
{
    NSLog(@"MyQuery: %@",query);
    // Run query and indicate that it's executable
    [self runQuery:[query UTF8String] isQueryExecutable:YES];
    
    // AffectedRows property can be used to verify whether there were any changes or not after having executed a query.
}

-(NSArray *)loadDataFromDB:(NSString *)query
{
    NSLog(@"MyQuery: %@",query);
    // Run query and indicate that it's not executable
